...
resource "azurerm_web_application_firewall_policy" "RES_WAF_POLICY" {
  for_each = var.AZ_WAF_POLICY_NAME 
    ...
    managed_rules {
      dynamic "exclusion" {
        ...
        content {
            ...
            excluded_rule_set {
              ...
              dynamic "rule_group" {
                for_each = exclusion.value["rule_group"]
                content {
                  rule_group_name           = rule_group.value["rule_group_rule_group_name"]
                  excluded_rules            = rule_group.value["rule_group_excluded_rules"]
                }
              }                
            }
        }
      ...
      managed_rule_set {
        type    = "OWASP"
        version = each.value.waf_policy_crs_version
